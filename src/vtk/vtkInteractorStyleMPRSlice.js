...
function vtkInteractorStyleMPRSlice(publicAPI, model) {
    ...
    window.zoom = model.zoomManipulator
    ...
    publicAPI.setVolumeMapper = (mapper) => {
        if (superSetVolumeMapper(mapper)) {
            ...
            if (mapper) {
                ...
                // console.log(mapper,'123123123123123');
     ...
     */
    // in world space
    publicAPI.setSliceNormal = (normal, viewUp = [0, 1, 0], iCenter = null) => {
        const renderer = model.interactor.getCurrentRenderer()
        const camera = renderer.getActiveCamera()
        ...
        if (model.volumeMapper) {
            ...
            let volumeCoordinateSpace = vec9toMat3(mapper.getInputData().getDirection())
            ...
            let center = iCenter || camera.getFocalPoint()
            ...
            if (Number.isNaN(dist) || dist === undefined) {
                ...
                const diagonal = Math.sqrt(
                    vtkMath.distance2BetweenPoints([bounds[0], bounds[2], bounds[4]], [bounds[1], bounds[3], bounds[5]])
                )
