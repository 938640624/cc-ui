...
function vtkInteractorStyleMPRWindowLevel(publicAPI, model) {
    ...
    publicAPI.windowLevelFromMouse = ([mx, my]) => {
        const range = model.volumeMapper.getMapper().getInputData().getPointData().getScalars().getRange()
    ...
    publicAPI.getWindowLevel = () => {
        const range = model.volumeMapper.getProperty().getRGBTransferFunction(0).getMappingRange().slice()
        return toWindowLevel(...range)
    }
    publicAPI.setWindowLevel = (windowWidth, windowCenter) => {
        const lowHigh = toLowHighRange(windowWidth, windowCenter)

        model.volumeMapper.getProperty().getRGBTransferFunction(0).setMappingRange(lowHigh.lower, lowHigh.upper)
    }
...
export function extend(publicAPI, model, initialValues = {}) {
    ...
    macro.setGet(publicAPI, model, ['volumeMapper', 'onLevelsChanged', 'levelScale'])
