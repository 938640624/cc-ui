<template>
    <div class="axis-rotate-model-page">
        <cc-header :headtitle="'AxisRotateModel(小视角旋转模型)'" />
        <cc-scroll>
...
export default {
    ...
    methods: {
        ...
        initView() {
            ...
            let widgetManager, orientationWidget, axes
            let coneMapper, coneActor
            // 圆锥体，辅助线
            let coneSource
            // 鼠标事件，渲染窗口
            ...
            function initAxes() {
                // vtkAxesActor：混合的2D、3D的actor，用于表示场景中3D，可以自定义形状，默认为轴尖端的几何形状
                axes = vtkAxesActor.newInstance()
                // vtkOrientationMarkerWidget：方向标记小部件，用于操作标记道具
