...
export function multipleBooleanCalculate(rule, arr) {
  switch (rule) {
    case '&&': return arr.every((item) => {
      const left = item[0]
      const operator = item[1]
      const right = item[2]
      return booleanCalculate(left, operator, right)
    })
    case '||': return arr.some((item) => {
      const left = item[0]
      const operator = item[1]
      const right = item[2]
      return booleanCalculate(left, operator, right)
...
// 用法1: isManualDeblock传false,func返回的是一个promise对象,那么promise对象完成后才可以触发下次点击
// 用法2: isManualDeblock传true,func的最后一个参数是done函数,执行done后才可以触发下次点击
export function ignoreMultiClick(func, isManualDeblock = false) {
  let isLock = false
 ...
 */
export function traverse(treeData, func, childrenKey = 'children') {
  for (const item of treeData) {
    func(item)
    const children = item[childrenKey]
    if (children && children.length) {
      traverse(children, func, childrenKey)
    }
  }
}
