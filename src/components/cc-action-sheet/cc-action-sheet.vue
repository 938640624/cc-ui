<template>
    <div class="cc-action-sheet">
        <cc-popover :visibleContent="visibleContent" @visible_content="handleVisibleContent">
            <cc-button-group>
                <template v-if="actionSheet.title">
                    <cc-button textColor="#333" fontSize="20px" :backgroundColor="backgroundColor" :border="border">
                        {{ actionSheet.title }}
                    </cc-button>
                </template>
                ...
                <template v-else-if="actionSheet.pickerStyle">
                    <div class="pickerStyle" @click="handleButtonCancel">取消</div>
                </template>
            </cc-button-group>
        </cc-popover>
        <cc-toast :toolVisible="toolVisible" :popoverText="popoverText" />
    </div>
</template>
<script>
