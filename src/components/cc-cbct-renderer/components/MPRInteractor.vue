<template>
    ...
    >
        <g class="distance">
            <Distance
                v-for="(d, index) in distances.filter((e) => e.isShow)"
                :key="index"
                :point1="d.point1"
                :point2="d.point2"
                :distance="d.distance"
        ...
        <g :transform="viewTransform" v-if="!distance">
            <!-- Y line -->
...
import { radians2degrees } from '@/helper/utils/math'
import { vec2, glMatrix, vec3 } from 'gl-matrix'
...
export default {
    ...
    computed: {
        sliceIntersection() {
            return this.getSliceIntersection()
        },
        slice() {
            return this.index === 'left' ? 0 : this.index === 'top' ? 2 : 1
        },
        volume() {
            return this.volumes[0]
        },
        bounds() {
            return this.volume.getBounds()
        },
        distance() {
